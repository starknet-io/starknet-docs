...
While running a node already contributes to the network's decentralization, becoming a validator allows you to earn rewards while taking on [additional responsibilities](/secure/quickstart/node-operators/attesting-to-blocks).
...
Besides [the prerequisites for running a full node](/secure/quickstart/node-operators/running-a-node#prerequisites), becoming a Starknet validator requires three accounts deployed on either Sepolia or Mainnet. To follow this guide, these account should be configured as follows:
