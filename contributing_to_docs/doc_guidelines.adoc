...
To use a local YAML file, add it to the `/<module>/attachments/` directory, and include it in your module. For example:

[source,yaml]
----
\include::attachment$install-config.yaml[]
----
...
If you are inserting an inline image that is not part of the `common-attributes.adoc`
