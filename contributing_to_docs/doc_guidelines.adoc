...
When you create new files, you must add them to the `/modules/Starknet/<module>/nav*.adoc` file so
that the build system can render them in the table of contents.
...
For style guidance, see xref:starknet_docs_style_guide.adoc#titles_and_section_headings[].
...
// * For the output of commands use `[source,text]`, such as with the following example output:
// +
// ....
// [source,text]
// ----
