---
title: "core::iter::traits::iterator::Iterator"
---

A trait for dealing with iterators.
This is the main iterator trait. For more about the concept of iterators
generally, please see the [module-level documentation](./core-iter). In particular, you
may want to know how to [implement `Iterator`](./core-iter).
...
Returns [`None`](./core-option#none) when iteration is finished. Individual iterator
implementations may choose to resume iteration, and so calling `next()`
again may or may not eventually start returning [`Some(Item)`](./core-option#some) again at some
point.

#### Signature

```rust
fn next(ref self: T) -> OptionItem>
```
...
This method will call `next` repeatedly until [`None`](./core-option#none) is encountered,
returning the number of times it saw [`Some`](./core-option#some). Note that `next` has to be
called at least once even if the iterator does not have any elements.
#### Overflow Behavior

The method does no guarding against overflows, so counting elements of
an iterator with more than [`Bounded::::MAX`](./core-num-traits-bounded-Bounded) elements either produces the
wrong result or panics.
#### Panics

This function might panic if the iterator has more than [`Bounded::::MAX`](./core-num-traits-bounded-Bounded)
elements.
#### Examples
...
This method will evaluate the iterator until it returns [`None`](./core-option#none). While
doing so, it keeps track of the current element. After [`None`](./core-option#none) is
returned, `last()` will then return the last element it saw.

#### Signature

```rust
fn last, +Destruct>(self: T) -> OptionItem>
```
...
times until [`None`](./core-option#none) is encountered.
`advance_by(n)` will return `Ok(())` if the iterator successfully advances by
`n` elements, or a `Err(NonZero)` with value `k` if [`None`](./core-option#none) is encountered,
...
`nth()` will return [`None`](./core-option#none) if `n` is greater than or equal to the length of the
iterator.

#### Signature

```rust
fn nth, +Destruct>(ref self: T, n: u32) -> OptionItem>
```
...
iterator.
`enumerate()` keeps its count as a [`usize`](./core-usize).
#### Overflow Behavior

The method does no guarding against overflows, so enumerating more than
`Bounded::::MAX` elements will always panic.
#### Panics
...
If they all return `false`, it returns [`None`](./core-option#none).
...
```rust
fn find[Output: bool], +Destruct, +Destruct, +Destruct>(ref self: T, predicate: P) -> OptionItem>
```
...
If either iterator returns [`None`](./core-option#none), `next` from the zipped iterator
will return [`None`](./core-option#none).
...
Since the argument to `zip()` uses [`IntoIterator`](./core-iter-traits-collect-IntoIterator), we can pass
anything that can be converted into an [`Iterator`](./core-iter-traits-iterator-Iterator), not just an
[`Iterator`](./core-iter-traits-iterator-Iterator) itself. For example:
```rust
...
Note that we needed the `: Array` on the left-hand side.
Using the 'turbofish' instead of annotating `doubled`:
```rust
let doubled = array![1, 2, 3].into_iter().map(|x| x * 2).collect::>();
...
```rust
fn peekable(self: T) -> PeekableItem>
```
...
```rust
fn sum, +Destruct, +Sum>(self: T) -> IteratorItem
```
...
```rust
fn product, +Destruct, +Product>(self: T) -> IteratorItem
```
...
Since the argument to `chain()` uses [`IntoIterator`](./core-iter-traits-collect-IntoIterator), we can pass
anything that can be converted into an [`Iterator`](./core-iter-traits-iterator-Iterator), not just an
[`Iterator`](./core-iter-traits-iterator-Iterator) itself. For example, arrays implement
[`IntoIterator`](./core-iter-traits-collect-IntoIterator), and so can be passed to `chain()` directly:
```rust
let a = array![1, 2, 3];
let b = array![4, 5, 6];
