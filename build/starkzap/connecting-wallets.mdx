...
![Wallets hero](/assets/starkzap/wallets-signers.png)
...
- **Building a game?** → Use **Cartridge** (web: `@cartridge/controller`; React Native: `starkzap-native` + [React Native Integration](/build/starkzap/react-native))
...
<Info>
  ...
  **For gasless transactions:** Configure [AVNU Paymaster](/build/starkzap/integrations/avnu-paymaster) separately to sponsor transaction fees.
</Info>

```typescript
import { StarkZap, StarkSigner, OpenZeppelinPreset } from "@starkware-ecosystem/starkzap";
...
<Info>
  For detailed Privy integration instructions, see the [Privy Integration Guide](/build/starkzap/integrations/privy).
</Info>

**Option A: Provide your backend URL (recommended for mobile/web clients)**

```typescript
import { StarkZap, PrivySigner, ArgentXV050Preset } from "@starkware-ecosystem/starkzap";
...
```typescript
import { PrivySigner } from "@starkware-ecosystem/starkzap";
...
[Cartridge](https://cartridge.gg) is specialized for gaming applications and provides social login and passkey authentication.

- **Web:** opens a Controller popup (install `@cartridge/controller`).
- **React Native / Expo:** in-app browser / deep-link **session** flow via `starkzap-native` — register the native Cartridge adapter before connecting; see [React Native Integration](/build/starkzap/react-native).
...
<Info>
  **Key advantage:** Cartridge can sponsor **policy-matching** transactions through its paymaster/session stack after users approve policies. Other calls may still require user fees or extra steps. See the [Cartridge Controller Integration Guide](/build/starkzap/integrations/cartridge-controller).
</Info>
...
```typescript
import type { SignerInterface } from "@starkware-ecosystem/starkzap";
...
```typescript
import type { AccountClassConfig } from "@starkware-ecosystem/starkzap";
...
- Learn about [Executing Transactions](/build/starkzap/transactions)
- Explore [ERC20 Token Operations](/build/starkzap/erc20)
- Set up [Staking & Delegation](/build/starkzap/staking)
