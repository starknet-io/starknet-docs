...
Starkzap supports **confidential** (privacy-preserving) balances and transfers through the **[Tongo](https://www.tongo.cash/)** protocol. Using the Tongo SDK, you can fund a confidential account from public ERC20, transfer between confidential accounts without revealing amounts, and withdraw back to a public Starknet address.

**What users can do with confidential:**
- **Fund a confidential account** - Convert public ERC20 into confidential balance (approval is included in the builder)
- **Transfer confidentially** - Send to another confidential account by recipient public key; amounts are hidden on-chain
- **Withdraw to public** - Convert confidential balance back to ERC20 and send to a Starknet address
- **[Tongo](https://www.tongo.cash/)-specific** - Ragequit (exit full balance) and rollover (activate pending balance)

<Info>
  The [Tongo](https://www.tongo.cash/) private key is separate from the Starknet wallet key. Your app must create and hold a `TongoConfidential` instance with the user's Tongo key and the correct Tongo contract address for the chain.
</Info>
...
Use the [Tongo](https://www.tongo.cash/) protocol documentation or deployment list for the correct `contractAddress` per chain. There is no wallet-level "register confidential"; you pass the `TongoConfidential` (or any `ConfidentialProvider`) into the tx builder methods.
...
- [Transactions](/build/consumer-app-sdk/transactions) — Execution options
- [Tx Builder](/build/consumer-app-sdk/tx-builder) — Batching and builder method list
