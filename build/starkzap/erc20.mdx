---
title: "Stablecoins & Token"
description: "Check balances, transfer tokens, and work with amounts using type-safe primitives"
icon: "credit-card"
---

![Bitcoin, Stablecoins, and Token hero](/assets/starkzap/bitcoin-stablecoins-token.png)
...
```typescript
import { getPresets } from "@starkware-ecosystem/starkzap";
...
```typescript
import { getTokensFromAddresses } from "@starkware-ecosystem/starkzap";
...
console.log(balance.toFormatted(true)); // "STRK 150.25" (compressed, max 4 decimals)
...
```typescript
import { Amount } from "@starkware-ecosystem/starkzap";
...
```typescript
const amount = Amount.parse("1500.50", 18, "ETH");

amount.toUnit(); // "1500.5"            — human-readable string
amount.toBase(); // 1500500000...0n     — raw bigint for contracts
amount.toFormatted(); // "ETH 1,500.5"       — locale-formatted with symbol
amount.toFormatted(true); // "ETH 1,500.5"       — compressed (max 4 decimals)
...
// Fee is paid in the fee token (ETH or STRK depending on fee mode).
// Compare transfer amount and fee separately against their respective balances.
if (balance.lt(transferAmount)) {
  throw new Error("Insufficient STRK balance for transfer");
}

// Check fee token balance separately
const feeBalance = await wallet.balanceOf(ETH);
const feeAmount = Amount.fromRaw(estimatedFee.overall_fee, ETH);
if (feeBalance.lt(feeAmount)) {
  throw new Error("Insufficient ETH balance for fees");
}
```

## Next Steps

- Learn about [Staking & Delegation](/build/starkzap/staking)
- Explore [Transaction Builder](/build/starkzap/transactions#transaction-builder) for complex operations
