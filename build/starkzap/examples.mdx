...
<Tabs>
  <Tab title="Flappy Bird (Web)">
    We integrate the **Cartridge Controller** — a wallet — with **session keys** (so users don’t sign each transaction) and a **paymaster** (gasless), all thanks to native **account abstraction** on Starknet. The demo game contract is already deployed; the app calls it on-chain and shows a **leaderboard** of scores. Live Demo is available here (to add).

    ### Controller via SDK: session keys + paymaster

    - **Controller = wallet:** The Cartridge Controller is the user’s wallet (social login / passkey) to connect to Starknet. We connect it through the SDK with **policies** that define which smart-contract calls the session may make.
    ...
    - **Paymaster:** Matching calls are **sponsored** (gasless); otherwise the example falls back to user-paid gas. In such a case users would need to fund their account.
