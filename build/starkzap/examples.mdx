...
Starkzap repository includes example applications in the `examples/` directory:
...
<Tabs>
  <Tab title="Flappy Bird (Web)">
    Game example: StarkZap + **Cartridge Controller**, **policies**, and **paymaster** (gasless). The on-chain contract is assumed already deployed; the app calls it (start game, increment score, end game) and reads high score / leaderboard.
    ...
    Open **`https://localhost:5173`** (HTTPS required for Cartridge). Click **Connect Controller**, sign in, then play; each pipe increments score on-chain (sponsored when the account supports SNIP-9).

    ### StarkZap + Controller

    - **SDK:** `StarkSDK({ network: "sepolia" })`, then `sdk.onboard({ strategy: OnboardStrategy.Cartridge, cartridge: { policies } })`.
    - **Policies:** One entry per contract, all methods listed (e.g. `start_new_game`, `increment_score`, `end_game`). Policies define what the session can call; only matching calls are paymastered.
    - **Paymaster:** Sponsored (gasless) when the Cartridge account is SNIP-9 compatible; otherwise the example falls back to user-paid gas.
    - **Contract:** We assume the game contract is already deployed. The app only **calls it on-chain** (execute + view) via `wallet.execute()` and read-only `Contract` + `wallet.getProvider()` (both from StarkZap re-exports).

    ### Snippets

    **Policies and connect:**
    ...
    **Execute (sponsored or user_pays):**
