...
The SDK name is **starkzap** (install: `npm install starkzap`, import: `from "starkzap"`). The repository includes example applications in the `examples/` directory:

1. **Flappy Bird (Web)** — Cartridge Controller, policies, and paymaster (gasless) on a game contract; contract assumed deployed, calls on-chain.
