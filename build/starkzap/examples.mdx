...
<Tabs>
  ...
  <Tab title="Server">
    ...
    | `/api/paymaster` | POST | No | Proxies the body to the AVNU paymaster, attaching the API key header. Returns the response as-is. |
    ...
    <Warning>
      This route raises the JSON body limit to 32 MB, because [STRK20](/build/starkzap/privacy/strk20) privacy proofs are megabytes of base64 and Express's 100 KB default rejects them with an HTML 413 before any route runs — which surfaces to the caller as "non-JSON response" rather than a size problem. Any proxy you put in front of a paymaster needs the same allowance.
    </Warning>
