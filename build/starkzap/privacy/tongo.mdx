---
title: Tongo confidential transfers
sidebarTitle: Tongo
...
<Warning>
  Use at most **one** confidential operation per transaction for a given account. Each one is proved against the balance and nonce read when it was built, so a second is already stale once the first applies, and the whole transaction reverts. Combining one confidential operation with ordinary calls, as above, is fine — send two confidential operations one after another.
</Warning>
...
You already hold all three arguments, and the SDK is already installed — it has to be, for confidential transfers to work at all. A second instance is equivalent to the one inside `TongoConfidential` rather than a rival to it: `Account` holds no chain state and reads its nonce and balance fresh on every call, so the two cannot disagree.
