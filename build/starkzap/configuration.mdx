---
title: Network, RPC, Staking, Bridging, and Logging
description: "Configure networks, RPC providers, staking, bridging, and logging for the SDK"
icon: cog
---
...
<AccordionGroup>
  ...
  <Accordion title="RPC">
    ...
    <Note>
      Plain `http://` is accepted only for loopback hosts (`localhost`, `127.0.0.1`, `[::1]`), which is what the `devnet` preset uses. Any other plain-http host is refused. If a device or emulator reaches a devnet or proxy over your LAN, set `allowInsecureHttp: true` on the SDK config. It applies to `rpcUrl`, the Cartridge URLs and the Privy `serverUrl`; the privacy module has its own flag. Never set it in a production build.
    </Note>
  </Accordion>
  ...
  <Accordion title="Bridging">
    ...
    const sdk = new StarkZap({
      ...
      bridging: {
        ...
        layerswapApiKey: "YOUR_LAYERSWAP_API_KEY", // for Layerswap routes + token discovery
      },
    });
    ```
    ...
    - `layerswapApiKey`: Required for Layerswap routes and Layerswap token discovery
    - `layerswapBaseUrl`: Optional Layerswap API base URL (defaults to `https://api.layerswap.io`)
    - `layerswapAllowedContracts`: Starknet contracts Layerswap may call in a withdrawal besides the bridge token. Unset, only the token transfer is signed. See [Bridging](/build/starkzap/bridging) before listing anything.
    ...
    <Warning>
      Layerswap routes and Layerswap token discovery require `bridging.layerswapApiKey`. Keys are environment-scoped — use a Mainnet key with Starknet Mainnet and a Testnet key with Starknet Sepolia.
    </Warning>
  ...
  <Accordion title="Logging">
    Configure optional SDK diagnostics when you need visibility into wallet, bridge, and transaction flows during development or debugging. Logging is opt-in and the SDK stays silent by default.
    ...
    StarkZap accepts any console-compatible logger with `debug`, `info`, `warn`, and `error` methods. You can also filter logs inside the SDK with `logLevel`:
    ...
    - Omit `logging` to keep the SDK completely silent
    - Omit `logLevel` to forward all log levels to your logger
    - Supported levels are `trace`, `debug`, `info`, `warn`, `error`, `fatal`, and `silent`
  </Accordion>
</AccordionGroup>
...
interface SDKConfig {
  ...
  rpcUrl?: string;
  chainId?: ChainId;
  allowInsecureHttp?: boolean; // accept plain http:// off loopback; dev only
...
interface BridgingConfig {
  layerZeroApiKey?: string; // OFT/OFT-migrated routes
  layerswapApiKey?: string; // Layerswap routes + token discovery
  layerswapBaseUrl?: string; // defaults to https://api.layerswap.io
  layerswapAllowedContracts?: readonly Address[]; // extra contracts Layerswap may call; unset = transfer only
...
- Configure [Bridging](/build/starkzap/bridging) for Ethereum/Solana deposits into and withdrawals out of Starknet
