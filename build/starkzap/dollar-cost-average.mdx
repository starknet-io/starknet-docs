...
<Info>
  `AvnuDcaProvider` (and `AvnuSwapProvider`, used by `previewCycle`) require the optional peer dependency `@avnu/avnu-sdk` (`npm install @avnu/avnu-sdk`). It is loaded lazily on first use, so the first AVNU DCA call throws an install hint if the package is missing. Ekubo providers need no extra dependency. See [Installation](/build/starkzap/installation#peer-dependencies-by-feature).
</Info>
...
### "requires optional peer dependency \"@avnu/avnu-sdk\""

An AVNU provider is selected but `@avnu/avnu-sdk` is not installed. Run `npm install @avnu/avnu-sdk`, or use the Ekubo providers (no extra dependency) instead.
