...
For **recurring buys (Dollar-Cost Averaging)**, see [DCA](/build/starkzap/dollar-cost-average).
...
<Info>
  `AvnuSwapProvider` requires the optional peer dependency `@avnu/avnu-sdk` (`npm install @avnu/avnu-sdk`). It is loaded lazily on the first quote/swap, so the first call throws an install hint if the package is missing. `EkuboSwapProvider` needs no extra dependency. See [Installation](/build/starkzap/installation#peer-dependencies-by-feature).
</Info>

**At connect / onboard:**

```typescript
import { StarkZap, AvnuSwapProvider, EkuboSwapProvider, ArgentPreset } from "starkzap";
...
See [Bitcoin, Stablecoins, and Tokens](/build/starkzap/erc20#token-presets) for more on token presets.
...
const tx = await wallet.swap(
  ...
  { feeMode: { type: "paymaster" } } // optional execution options
);
await tx.wait();
```
...
For **Dollar-Cost Averaging (recurring buys)** with AVNU or Ekubo, see [DCA](/build/starkzap/dollar-cost-average).
...
The AVNU provider is selected but `@avnu/avnu-sdk` is not installed. Run `npm install @avnu/avnu-sdk`, or use `EkuboSwapProvider` (no extra dependency) instead.
