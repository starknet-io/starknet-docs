...
<Update label="Starknet v0.14.1 (TBD)">
  Starknet v0.14.1 is scheduled to arrive on Testnet in TBD
  
  This version includes the following changes:

  - Prover optimization: compiled class hashes will now be computed with the Blake hash function rather than Poseidon.
    ...
    - The feeder gateway will add such classes whose compiled class hash was updated to the `declared_classes` list in the state update response.
