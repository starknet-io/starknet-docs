...
- $\sigma$ is the percentage of STRK staked out of total STRK supply

Annual reward percentages (APR) can therefore be calculated using the following formulas:

- For STRK: $100 \cdot \frac{M}{\sigma}$
...
<Note>
  [As previously described](#epochs), stakers that enter the protocol on epoch $i$ will start getting rewards only on epoch $i+k$, and stakers that signal an intent to exit the protocol on epoch $i$ will still get rewards until epoch $i+k-1$.
</Note>
