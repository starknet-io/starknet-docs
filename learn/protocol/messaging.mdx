...
Starknet's ability to interact with L1 is crucial, and is used by bridges (e.g., for depositing your tokens in the L1 bridge contract, and automatically triggers the minting of the same token on L2), [DeFi pooling](https://starkware.co/resource/defi-pooling/) and [more](https://www.starknet.io/en/ecosystem/dapps). Messaging — both [from L1 to L2](/learn/protocol/messaging#l2-→-l1-messages) and from [L2 to L1](/learn/protocol/messaging#l2-→-l1-message-protocol) — is the mechanism that enables this interaction.
...
<Frame caption="L2→L1 message mechanism">
  <img src="/assets/images/f1564f0a-l2l1.png" />
</Frame>
...
The fee itself is calculated in the [same manner](/learn/protocol/fees#overall-fee) as "regular" L2 transactions.
...
* *h* is the [Pedersen](/learn/protocol/cryptography#pedersen-hash) hash (note that since we're hashing an array, the # of inputs needs to be appended to the hash).
