...
[Similar to other blockchains](#background), Starknet uses the STRK token for [paying fees to enable operation of the network, maintaining and securing the network by enabling staking for consensus, and deciding on Starknet's values and technology goals by voting for governance proposals](#purposes), of which [ten billion tokens are planned to or have already been distributed](#supply_and_distribution),  as well as, further tokens minted through an inflation mechanism, as further described under the [<u>Supply and distribution</u>](https://docs.google.com/document/d/1vzSgYXNC3pzdDHJp0LdflGxZX-pn9x9YIBqrCxZkKc0/edit?tab=t.0#heading=h.sethkpowcoqs) section.
