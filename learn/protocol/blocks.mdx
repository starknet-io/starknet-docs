...
- For transactions: `h(transaction_hash, signature)` for the `i`'th transaction included in the block, where [transaction_hash is calculated as follows](/learn/cheatsheets/transactions-reference#hash-calculation) and `h` is the [Poseidon hash function](/learn/protocol/cryptography#poseidon-hash). Note that the signature is itself a (possibly empty) array of field elements.
