...
h(
    ...
    s,
    deployed_or_replaced_address1, ... , deployed_or_replaced_addresss,
    d,
    declared_or_migrated_class_hash1, declared_or_migrated_compiled_class_hash1..., declared_or_migrated_class_hashd, declared_or_migrated_class_hashd,
    1,
    0,
    n,
...
- `ci,ki,1, vi,1, … , ki,mi,vi,mi` are `mi` updates `(ki,1, vi,1), … ,(ki,mi, vi,mi)` to contract addresses `ci` . Where `ki,j` is the `j`-th key and `vi, j` is the `j`-th new value
