...
* RPC v0.9 is introduced, removing the `REJECTED` transaction status, changing the behavior of the `RECEIVED` and `ACCEPTED_ON_L2` transaction statuses, adding new `CANDIDATE`, and `PRE_CONFIRMED` transaction statuses, and renaming the `PENDING` block to `PRE_CONFIRMED` block 

* The feeder gateway URL is now `feeder.\{chain\}.starknet.io/feeder_gateway` (while the gateway URL remains `\{chain_name\}.starknet.io/gateway`), adding a new `get_preconfirmed_block` endpoint, deprecating the `get_oldest_transaction_age` and `get_number_of_transactions_in_backlog` endpoints, and changing the behavior of the `get_transaction_status` endpoint
...
* Sequencers now maintain a mempool, decouping the transaction order of arrival from the transaction ordering in blocks
