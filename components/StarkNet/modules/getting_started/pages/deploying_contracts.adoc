...
In order to interact with the Starknet main chain or any testnets, you'll need an RPC provider. Please refer to https://docs.alchemy.com/reference/starknet-api-quickstart/?a=starknet-docs[Alchemy's quickstart guide] to start developing with Starknet APIs!
...
== Creating and compiling a contract
...
[NOTE]
====
The above command may fail if you are using code that has already been declared by someone else.
Please make sure to add custom code to your contract to create a new contract class.
====
...
----
The transaction hash allows you to track when the network will have received your contract's code
. Once this transaction has moved to `PENDING`, you can deploy an instance of your contract.
