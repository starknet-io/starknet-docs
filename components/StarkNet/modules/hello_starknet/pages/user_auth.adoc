...
Save the new contract file as `+user_auth.cairo+`. You can find the full Cairo file https://www.cairo-lang.org/docs/_static/user_auth.cairo[here].
...
where `+${USER_AUTH_CLASS_HASH}+` is the value of class_hash. Don’t forget to set the `+STARKNET_NETWORK+` and `+STARKNET_WALLET+` environment variables and https://www.cairo-lang.org/docs/hello_starknet/account_setup.html#create-account[deploy an account contract] before running `+starknet deploy+`.
