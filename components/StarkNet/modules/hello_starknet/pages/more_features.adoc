...
An external function may get an array of field elements as an argument. In order to define an array named `a`, pass two consecutive arguments: `a_len` of type `felt` and `a` of type `felt*` (the first argument _must_ be named `a_len` if the second argument is named `a`). For example:
