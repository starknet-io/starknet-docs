...
API version:: The version of the API under which the class was deployed.
...
Keccak:: The xref:../Hashing/hash-functions.adoc#starknet-keccak[starknet_keccak], of the class's program. The class's program is the `.json` file that the StarkNet compiler outputs when you run the following command:
+
[source,shell]
----
