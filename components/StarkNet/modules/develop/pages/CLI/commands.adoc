...
`alpha-mainnet`:: Sets the StarkNet network to testnet

.Setting the environment using a command-line option

When you enter any command, include the the `--network` option. For example to use Mainnet, enter any command as follows:
...
.Setting the environment using an environment variable
...
When working with the CLI, you can manually set the read/write endpoints for the gateways that enable you to
interact with StarkNet, by including the `--feeder_gateway_url` and `--gateway_url` options.
