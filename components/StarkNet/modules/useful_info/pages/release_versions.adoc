...
Within StarkNet's deployment pipeline there are separate and distinct networks that operate independently of each other for testing before deployment. See xref:./index.adoc[here] for more information. 


[id="versions"]
== Upcoming version (v0.10.3)

Replace with: This is the currently deployed version on StarkNet Testnets. The Mainnet environment upgrade is planned for December 12.
