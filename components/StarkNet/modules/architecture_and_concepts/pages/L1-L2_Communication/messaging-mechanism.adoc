...
NOTE: As the message being sent needs to be written to L1 storage (in the StarkNet Core contract) there is always a fixed 20k gas cost associated with sending an L2 to L1 message.
...
| `List` | `FieldElement`  | `FieldElement` | `FieldElement` | `FieldElement` |
|===
