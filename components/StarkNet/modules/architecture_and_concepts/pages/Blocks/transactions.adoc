...
== Deploy transaction (discontinued)

[IMPORTANT]
====
The deploy transaction has now been removed in the current release of StarkNet. To deploy new contract instances, you can use the `deploy` syscall. For more information, see xref:../Contracts/contract-classes.adoc[contract classes].
====
...
* `SN_GOERLI2` for StarkNet's developer only testnet.