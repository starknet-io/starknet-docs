...
* Contract instances: A mapping between addresses and contract's state.
* xref:architecture_and_concepts:Contracts/contract-classes.adoc[Contract classes]: A mapping
between class hash and the class definition.

A contract state consists of:

* xref:Contracts/class-hash.adoc[Class hash]: Defines the functionality.
* xref:../Contracts/contract-storage.adoc[Contract storage]: A key-value mapping where the
key/values are field elements.
...
Note that while on Ethereum, only EOAs can send transactions and consequently have a non-zero
nonce, on Starknet, only xref:Account_Abstraction/introduction.adoc#account_abstraction[account
contracts] can send
transactions and have a non-zero nonce.
====
...
* `class_hash` is the hash of the contract's definition discussed xref:documentation:architecture_and_concepts:Contracts/class-hash.adoc[here]
...
* `h` is the xref:../Hashing/hash-functions.adoc#pedersen_array_hash[Pedersen] hash function.

[id="classes_tree"]
=== The classes tree

The classes tree encodes the information about the declared xref:../Contracts/contract-classes.adoc#[class] in the state of Starknet. It maps xref:../Contracts/class-hash.adoc#cairo1_class[class hashes] to their
