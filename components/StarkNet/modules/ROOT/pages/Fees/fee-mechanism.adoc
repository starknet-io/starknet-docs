...
In this section, we will review StarkNet Alpha 0.8.0 fee mechanism. If you want to skip the motivation and deep dive into the mechanism, you can skip directly to the final xref:./fee-mechanism.adoc#overall-fee[formula].
...
* xref:./fee-mechanism.adoc#computation[computational complexity]: the heavier the transaction, the larger its portion in the proof verification cost.
* xref:./fee-mechanism.adoc#on-chain-data[on chain data]: L1 calldata cost originating from xref:../Data_Availability/on-chain-data.adoc[data availability] and L2→L1 messages.
