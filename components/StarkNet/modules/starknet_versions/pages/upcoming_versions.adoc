...
In this section we will cover design changes up to the Testnet/Mainnet deployment.
...
You can find the specification of our Poseidon hash function https://github.com/starkware-industries/poseidon[here].

#### State commitment

The changes elaborated xref:#commitment[above] will be slightly modified as follows:
...
* If the classes tree is none empty, the commitment will be computed by `h(global_tree_root, classes_root, commitment_version)` where `h`
...
In addition to the changes explained xref:#ocd[here], we will have an additional section for the newly declared (Cairo 1) classes in the block. This section will contain:
