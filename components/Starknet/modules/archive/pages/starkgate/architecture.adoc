...
. The sequencer waits for enough L1 block confirmations to fill its quota to run before the corresponding deposit transaction is initiated on L2. During this period of time, the status of the L2 deposit transaction is xref:components-and-concepts:transactions.adoc#transaction_lifecycle[`NOT_RECEIVED`].
...
The status of the deposit request is now xref:components-and-concepts:transactions.adoc#transaction_lifecycle[`ACCEPTED_ON_L2`].
