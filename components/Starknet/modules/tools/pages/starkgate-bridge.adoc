...
. A call to the L1 xref:starkgate_function_reference.adoc#deposit[`deposit`] function initates a deposit.
...
. A call to the L2 xref:starkgate_function_reference.adoc#initiate_withdraw[`initiate_withdraw`] function initiates a withdrawal.
. The function does the following:
...
After the withdrawal message has been recorded on the Starknet Core Contract, anyone can finalize the transfer on L1 from the bridge back to the user, by calling the xref:starkgate_function_reference.adoc#withdraw[`withdraw`] function.
