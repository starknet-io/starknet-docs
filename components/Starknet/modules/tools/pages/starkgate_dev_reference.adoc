...
link:https://github.com/starkware-libs/starkgate/blob/dev/src/solidity/StarknetTokenBridge.sol[StarknetTokenBridge.sol]:: The StarkGate multi-token bridge implementation. #If you are a dApp developer, you can use the functions in this contract to implement all currently supported single bridges, such ETH and USDC.#
...
This transaction enables dApps to improve the user experience by enabling a deposit transaction to trigger a subsequent actions, For example, you can deposit funds and at the same time transfer those funds to another address. Although this is two transactions, `depsoitWithMessage` lets you make this into a single user action.
...
.`DepositWithMessage` event attributes #Parameters need descriptions#
