...
#What is included in this message? Why would someone use this instead of `deposit`?#
...
`uint256 _amount_`:: The amount of the deposit.
`uint256 _l2Recipient_`:: The L2 address of the recipient.
`uint256[] calldata _message_`:: The message attached to the deposit. #What is included in the message?#
...
.Event parameters #Parameters need descriptions#
...
Sends a request to StarkGate to cancel a deposit sent with xref:#depositWithMessage[`depositWithMessage`].

Similar to xref:#depositCancelRequest[`depositCancelRequest`].
...
For more information, see xref:#depositWithMessageCancelRequest[`depositWithMessageCancelRequest`].
