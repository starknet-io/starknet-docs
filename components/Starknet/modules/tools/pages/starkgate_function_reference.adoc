...
If `on_receive` returns `true`, then the operation succeeded. If it returns `false`, the transaction is reverted.
//
// ##----
// fn on_receive(
// self: @ContractState,
...
`address indexed _token_`:: Same as the parameter `address _token_`.
