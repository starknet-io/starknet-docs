...
* Starkli and Scarb are installed correctly. Ensure that the following commands show the version information for Starkli and Scarb:
+
[source, bash]
----
starkli --version
...
The `fetch` command supports both Argent X and Braavos smart wallets. Make sure your wallet address is already deployed and input the following command to create and save the account descriptor file:
