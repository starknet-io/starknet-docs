...
A block context can be specified via the `latest` or `pending` tags, where the former refers to the latest accepted on L2 block and the latter refers to the xref:architecture-and-concepts:network-architecture/transaction-life-cycle.adoc#the-pending-block[pending block].
====
...
- `contract_abi`* - a path to a JSON file containing the xref:architecture-and-concepts:smart-contracts/contract-abi.adoc[abi] of the receiving contract on L2
