...
StarkGate architecture, as presented in Figure 1 xref:#StarkGate_architecture_diagram[_StarkGate architecture diagram_], is composed of pairs of L1 and L2 StarkGate contract instances that communicate via Starknet’s L1-L2 messaging mechanism. 
...
Each pair of bridges correspond with the relevant ERC-20 contracts on Ethereum and Starknet, as described in table 1 xref:#bridge_explanation_table[].

._StarkGate architecture_
[#StarkGate_architecture_diagram]
image::sg-architecture.png[]

._Components of a bridge for an individual token_
...
* StarkGate solidity bridge contract instance for ERC-20 tokens which is supported in `StarknetTokenBridge.sol`.
| *L2*
a|
...
You can permissionlessly add support for ERC-20 tokens to the multi-token bridge using the `enrollTokenBridge` function. For more information, go to https://docs.starknet.io/starkgate/adding-a-token/[Adding a token].

[#StarkGate_manager_and_registry]
== The StarkGate Manager and Registry

StarkGate includes the following administration components, which are L1 contracts:

// [horizontal]
* The _StarkGate Manager_: is responsible for adding tokens to the multi-bridge.
* The _StarkGate Registry_:
** Contains the addresses of all supported bridges and tokens.
...
* xref:adding-a-token.adoc[Adding a token]
...
** https://github.com/starknet-io/starkgate-contracts/tree/cairo-1[L1 and L2 contracts on GitHub]
