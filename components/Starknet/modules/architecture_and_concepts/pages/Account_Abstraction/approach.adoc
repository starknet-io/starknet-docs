...
// I don't understand this last sentence. You say that every contract has a nonce, which is unique, and that every transaction sent from the same account must be unique and match the account. It sounds like you're saying if the contract's nonce is `1`, then every transaction that comes from this account must also be `1`, which is obviously not true. So what do you mean by "match"? An example would be helpful.
