...
.Description
Gets the hash of the given block.

.Arguments

.Return values
Returns the hash of the given block when `block_number` is in the range `[current block - 10,
v_0_12_0_first_block]`.

If `block_number` > `current -10`, an error will be returned. Otherwise returns zero.
