...
The following types are smaller than 252 bits. For these types, each value is serialized as a a single-member list that contains one `felt252` value.
...
* The first `felt252` value contains the 128 most significant bits, usually referred to as the low part of the original `u256` value.
* The second `felt252` value contains the 128 least significant bits, usually referred to as the high part of the original `u256` value.
...
Contains 31-byte chunks of the byte array. Each `felt252` value has exactly 31 bytes. If the string has less than 31 bytes, then this array is empty.
