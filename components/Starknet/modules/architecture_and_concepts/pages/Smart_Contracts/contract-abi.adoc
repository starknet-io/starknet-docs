...
You may not want to nest enums when serializing the event. For example, if you write an ERC20 as a component (not a contract) that is pluggable anywhere, you may not want the contract to modify the keys of known events such as `Transfer`.
