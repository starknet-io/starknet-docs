...
Gets information about the currently executing block and the transactions in the block.
...
Returns the struct `ExecutionInfo`. `ExecutionInfo` is a container for information about the currently executing block and the transactions in the block.

.Contents of `ExecutionInfo`
[cols="2,3"]
|===
|`_block_info_: Box<BlockInfo>` a| Contains the following information about a block:
[horizontal,role="stripes_odd"]
`_block_number_: u64`:: The number of the block that is currently being executed.
`_block_timestamp_: u64`:: The timestamp showing the creation time of the block. #Is this in UTC time? Or seconds since the Unix epoch? Or something else?#
`_sequencer_address_: ContractAddress`:: The address of the Starknet sequencer contract.
|`_tx_info_: Box<TxInfo>`
a| Contains the following information about a transaction:
[horizontal,role="stripes_odd"]


|`_caller_address_: ContractAddress` |
|`_contract_address_: ContractAddress` |
|`_entry_point_selector_: felt252` |
...
// For 0.13.0, see https://github.com/starkware-industries/starkware/blob/a57dbc9e0a7ece680bae52dd53216b2332c74618/src/starkware/starknet/common/new_syscalls.cairo#L30.
