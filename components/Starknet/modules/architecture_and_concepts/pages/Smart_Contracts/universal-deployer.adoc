...
[source,cairo]
----
use openzeppelin::universal_deployer::interface::IUniversalDeployerDispatcher;
...
[CAUTION]
====
When deploying a contract that uses `get_caller_address` in the constructor calldata, remember that the UDC (and not the account) deploys that contract.
