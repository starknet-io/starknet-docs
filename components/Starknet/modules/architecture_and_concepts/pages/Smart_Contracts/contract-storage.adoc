...
The `#[storage]` attribute above the `Storage` struct declares that the content of this struct are part of the contract storage. The storage variables stored inside this struct can consist of a single felt, or it can be a mapping from multiple arguments to a tuple of felts or structs.
