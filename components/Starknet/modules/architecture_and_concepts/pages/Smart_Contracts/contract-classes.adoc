...
can be deployed via the xref:architecture_and_concepts:Smart_Contracts/system-calls.adoc#deploy[`deploy`]
system call.

To use the functionality of a declared class, without deploying an instance of that class, you
can use the xref:Smart_Contracts/system-calls.adoc#library_call[`library_call`] system call. This system call is an
