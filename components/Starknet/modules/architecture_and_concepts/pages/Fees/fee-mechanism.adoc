...
* xref:_on_chain_data[On chain data]: L1 calldata cost originating from xref:Data_Availability/on-chain-data.adoc[data availability] and L2→L1 messages.
...
The proof will be closed and sent to L1 when any of these components becomes full.
...
Notice we completely ignored the number of Cairo steps in this transaction performance estimation, as it is not the limiting factor.
====
