...
[horizontal,labelwidth="30",role=stripes-odd]
`uint256 _MAX_L1_MSG_FEE_`:: The maximum fee, in Wei, that Starknet accepts per single message.
...
Indicates if a pending message is associated with a given message hash and
returns `msg_fee + 1` if there is a pending message associated with the given 'msgHash', otherwise, returns 0.

// What is msg_fee? I don't see it anywhere in StarknetMessaging.sol.
// Why does it return `msg_fee + 1`? How is that value used?
