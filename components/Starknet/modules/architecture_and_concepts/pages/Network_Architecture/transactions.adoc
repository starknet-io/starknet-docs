...
The *Invoke* v3 transaction hash is calculated as a hash over the given transaction elements,
specifically:

[,subs="quotes"]
----
invoke_v3_tx_hash = _h_(
    "invoke",      // 1. A prefix that depends on the transaction type.
    version,       // 2. The transaction's version.
    sender_address, // 3. Sender address.
    _h_(tip, resource_bounds),      // 4. A hash of the fee-related fields (see `_hash_fee_related_fields()`'s docstring).
    _h_(paymaster_data), //   5. A hash of the paymaster data.
    chain_id,      // 6. The network's chain ID.
    nonce,      // 7. The transaction's nonce.
    _concatenate_(nonce_data_availability_mode, fee_data_availability_mode)  // 8. A concatenation of the nonce and fee data availability modes.
    account_deployment_data // 9. Transaction-specific additional data.
    _h_(calldata),
    class_hash
)
----
