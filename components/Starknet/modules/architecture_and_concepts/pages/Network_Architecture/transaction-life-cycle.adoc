...
. *Mempool validation:*
The mempool performs a preliminary validation on the transaction. It lists for each transaction which validate function the account contract calls. If the transaction is invalid, it does not proceed.
...
. *Execution:* The sequencer operation sequentially applies all transactions that passed the preliminary validation to the state, by calling the relevant function in the account contract that submitted each transaction, as follows:
+
--
* For `INVOKE` transactions, the sequencer calls the `+__execute__+` function.
* For `DEPLOY_ACCOUNT` transactions, the sequencer calls the `+__constructor__+` function.
* For `DECLARE` transactions, the sequencer does not execute any code in the account contract that submitted the transaction.
--
+
If a transaction fails during execution, it is included in the block with the status `REVERTED`.
