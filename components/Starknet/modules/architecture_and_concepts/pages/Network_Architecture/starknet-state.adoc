...
. Use an SDK to generate a compiled class hash from the Sierra. HOW?
. Add the Sierra code of the class to Starknet using a `DECLARE` transaction.
. The sequencer compiles the Casm from the Sierra code, stores it on the Starknet blockchain's storage, and generates a hash of the compiled class.
--
+
