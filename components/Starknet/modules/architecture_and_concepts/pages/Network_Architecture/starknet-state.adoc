...
. Use an SDK to generate a compiled class hash from the Sierra.
. Add the Sierra code of the class to Starknet using a `DECLARE` transaction.
. The sequencer compiles the Casm from the Sierra code, stores it on the Starknet blockchain's storage, and generates a hash of the compiled class. This hash should be identical to the hash that the SDK generated.
--
=====

The party that declares the contract signs the compiled class hash as part of the xref:Network_Architecture/transactions.adoc#declare_v2[`DECLARE`] transaction. If the transaction is included in a block, then the compiled class hash becomes part of the state commitment.
