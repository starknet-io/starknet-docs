...
Address `0x1` is another special contract address within StarkNet's framework. It functions as a storage space for mapping block numbers to their corresponding block hashes. The storage structure at this address is organized as follows:
