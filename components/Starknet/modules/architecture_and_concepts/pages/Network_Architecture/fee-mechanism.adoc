...
* xref:computation[Computational complexity]: A transaction's portion of the proof verification cost is proportional to its complexity.
...
The fee is denominated in #STRK#. Each transaction is associated with an estimate of the amount of gas used. Combining this estimate with the price of gas yields the estimated fee.
...
* Number of applications of each Cairo builtin. For example, five range checks and two Pedersen hashes.
