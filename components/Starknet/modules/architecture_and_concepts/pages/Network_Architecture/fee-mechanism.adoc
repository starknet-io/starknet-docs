...
[stem]
++++
F = \text{gas_price}\cdot\left(\max_k v_k w_k + \text{calldata_cost}\cdot\left(2n+2(m-2) + 3t + \sum\limits_{i=1}^t q_i +3\ell\right) - \text{contract_update_discount}\cdot n + 310 + \text{storage_write_cost}\cdot t\right)
++++
...
* stem:[$m$] is the number of values updated, not counting multiple updates for the same key. Note that stem:[$m\ge 2$] always holds since the sending account balance and the sequencer's balance are always updated.
...
* stem:[$\text{calldata_cost}$] is 550 gas per 32-byte word. This cost is derived as follows: 512 gas per 32-byte word for calldata, ~100gas for on-chain hashing that happens for every sent word, and 10% discount because the sequencer does not incur additional costs for repeated updates to the same storage slot within a single block, resulting in 0.9*612 = 550gas (rounded down).
* stem:[$310$] is the cost of updating the sender's balance, for the derivation of this number see xref:#storage_updates[].
...
[stem]
++++
\underbrace{\textit{gas_price}\left(\text{calldata_cost} \cdot 2n - \text{contract_update_discount}\cdot n\right)}_{\text{contract addresses +  new nonce and number of storage updates
}} \\

+ \\

\underbrace{\textit{gas_price} \cdot \left(\text{calldata_cost}(2(m-2))+310\right)}_{\text{storage updates}}
++++

where:

* stem:[$n$] is the number of unique contracts updated.
* stem:[$m$] is the number of values updated, not counting multiple updates for the same key. Note that stem:[$m\ge 2$] always holds since the sending account balance and the sequencer's balance are always updated. The sender's balance is treated separately (described below) and the sequencer's balance is ignored since it is touched by all the transactions within a block.
* stem:[\text{contract_update_discount}] is 312 gas, which is discounted for every updated contract, since out of the stem:[$2n$] words induced by updated contracts, stem:[$n$] are short, including at most stem:[$6$] non-zero bytes:
+
--
...
* stem:[$310$] is the cost of updating the sender's balance, and is derived by assuming the balance requires at most 12 non-zero bytes (which is enough for 1.2B ETH or STRK), resulting in the following discounted price: stem:[$0.9*612-(512-(16*12+20*4))=310$] (rounded down).
