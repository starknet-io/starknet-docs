...
[stem]
++++
F = \text{gas_price}\cdot\left(\max_k v_k w_k + \\
...
\text{message_calldata_cost}\cdot\left(3t + \sum\limits_{i=1}^t q_i\right) \\
 + \text{storage_write_cost}\cdot t\right)
++++
...
[stem]
++++
\textit{gas_price}\cdot \left(\text{da_calldata_cost} + \test{message_calldata_cost}\right)\cdot\left(3+\text{payload_size}\right)
++++
