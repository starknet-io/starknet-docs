...
[stem]
++++
F = \text{gas_price}\cdot\left(\max_k v_k w_k + \text{calldata_cost}\cdot\left(2n+2(m-1) + 3t + \sum\limits_{i=1}^t q_i +\ell\right) - \text{contract_update_discount}\cdot n - 240 + \text{storage_write_cost}\cdot t\right)
++++
...
* stem:[$m$] is the number of values updated, not counting multiple updates for the same key. Note that stem:[$m\ge 1$] since the sequencer balance is always updated (this is not charged for).
...
\underbrace{\textit{gas_price} \cdot \left(\text{calldata_cost}(2(m-1))-240\right)}_{\text{storage updates}}
++++
