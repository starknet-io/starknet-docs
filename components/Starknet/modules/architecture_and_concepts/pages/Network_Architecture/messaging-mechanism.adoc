...
[NOTE]
====
In an `l1_handler` transaction, as the transaction is not sent by an account, calling `get_caller_address()` or other functions related to the account will result in the address `0x0`.
====

.Supported versions of L1 Handler transaction
[cols=",,",]
|===
