...
Each of the commitment fields `event_commitment` and `transaction_commitment` are each the roots of a height 64 binary Merkle Patricia tree

[horizontal,labelwidth='20']
`event_commitment`:: is the root of a height 64 binary Merkle Patricia tree. The leaf at index stem:[$i$] corresponds to the hash of the stem:[$i'th$] event.
