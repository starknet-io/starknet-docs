[id="gas-and-transaction-fees-pre-v0.13.0"]
= Gas and transaction fees (Prior to Starknet 0.13.0)
:--auto-ids:
:stem: latexmath

[IMPORTANT]
====
For information on v0.13.0, see xref:architecture_and_concepts:Network_Architecture/fee-mechanism.adoc[].
====
...
[stem]
++++
F = \text{gas_price}\cdot\left(\max_k v_k w_k + c_w\left(2(n+m) + 3t + \sum\limits_{i=1}^t q_i +3\ell\right)\right)
++++
