...
`v3` transactions already provide STRK-denominated `max_amount` and `max_price_per_unit` for each of the three xref:fee_units[fee unit] (as part of the `resource_bounds` dictionary), while previous versions only provide a WEI-denominated overall `max_fee`. However, starting in Starknet v0.14.0, _all_ transactions will have specify STRK-denominated bounds for all three fee unit, as well as a STRK-denominated `tip`.

A necessary condition for a transaction to enter a block is that for each resource, `max_price_per_unit` is greater than or equal to the xref:gas_prices[block's corresponding price]. `tip` is a further addition to the `max_price_per_unit` for `l2_gas`, i.e. a sequencer will receive `(base_fee + tip) * l2_gas_used` STRK tokens for including the transaction in a block. 
