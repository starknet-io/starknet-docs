...
For more information, and for a higher-level syntax for emitting events, see xref:architecture:smart-contracts/starknet-events.adoc[Starknet events].
...
For more information, see Starknet's xref:network-architecture/l1<>l2-messaging.adoc[messaging mechanism].
