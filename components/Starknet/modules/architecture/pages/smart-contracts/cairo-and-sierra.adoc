...
Before xref:settings:upcoming-versions.adoc[Starknet Alpha v0.11.0] a developer would write contracts in Cairo 0 and compile them locally to Cairo assembly (or Casm for short).
...
Starting with Cairo 1.0, the contract class resulting from xref:smart-contracts/class-hash.adoc#cairo1_class[compiling Cairo 1.0] does not include Casm. Instead of Casm, it includes instructions in an intermediate representation called **S**afe **I**nt**er**mediate **R**epresent**a**tion, _Sierra_ for short.
