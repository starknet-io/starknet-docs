...
The following sections describe the procedures available for validators. The functions used in these procedures are summarized in the following table:
...
// | Staking
// | `stake`
// | No limit, sets caller as staking address

.2+.^| Increasing stake
| `approve`
| STRK
.2+.^| Address to be used as staking address
| `increase_stake`
| Staking
...
| `approve`
| STRK
.2+.^| Rewards address or staking address
| `claim_rewards` 
| Staking
...
[NOTE]
====
The inital stake must greater or equal to the xref:architecture-and-concepts:staking.adoc#protocol[minimum stake required for validators].
====
...
To intialize their stake, validators must first approve the transfer of STRK tokens to the staking contract by invoking the STRK contract's `approve` function with the following parameters:
...
One the transfer is approved, validators can use the same address to invoke the staking contract's `stake` function with the following paramters:
...
For an example of approving the transfer of 1 STRK to the staking contract on Sepolia, see xref:#intializing_stake[]. Once the transfer is approved, validators can use the same address to invoke the staking contract's `increase_stake` function with the following parameters:

. The validator's rewards or staking address
. The number of STRK tokens to be added

For example, the following can be used to increase an exisiting stake by 1 STRK:
