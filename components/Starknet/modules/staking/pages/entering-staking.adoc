...
* A Starknet-compatible xref:tools:block-explorers-monitoring-tools.adoc[block explorer] or xref:tools:devtools/interacting-with-starknet.adoc[CLI tool].
...
.. Navigate to the STRK contract by searching for xref:ROOT:chain-info.adoc#important_addresses[its address]
