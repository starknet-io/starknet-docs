...
== Introduction
Staking on Starknet involves locking STRK tokens in the staking protocol, contributing to network security and performance. Anyone holding STRK in their wallets can stake their tokens on Starknet either directly as a validator or indirectly as a delegator.

This guide will walk you through the various procedures enabled by the staking protocol to both validators and delegators, including:

.For validators

* xref:staking:entering-staking.adoc[Registering as validator]
