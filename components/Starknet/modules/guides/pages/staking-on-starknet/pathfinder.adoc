...
To claim your staking rewards, you must run one of the various xref:ecosystem:fullnodes-rpc-providers.adoc#full_nodes[Starknet full node implementations]. This installment of the series will therefore walk you though xref:downloading_a_snapshot[downloading a Pathfinder dataset snapshot] and using it to xref:running_the_node[run a Pathfinder full node].
