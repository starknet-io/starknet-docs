...
In this chapter, we will provide a step by step guide on how to stake your STRK tokens using Starknet blockchain explorer (https://voyager.online/[Voyager^] or https://starkscan.co/[Starkscan^]). We will also utilize the two address from the xref:staking-on-starknet/prerequisite.adoc[prerequisite] section to assign reward and operation addresses. 
