...
[NOTE]
====
To understand the role of the each account, read more about xref:components-and-concepts:staking.adoc#addresses[validator addresses in the staking protocol].
====
...
* An `sncast` account named `staker` holding at least xref:components-and-concepts:staking.adoc#protocol[the minimum required amount of STRK tokens] with its address exported using:
+
[source,terminal]
----
