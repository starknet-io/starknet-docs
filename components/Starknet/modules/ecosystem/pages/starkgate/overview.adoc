...
. The sequencer waits for enough L1 block confirmations to fill its quota to run before the corresponding deposit transaction is initiated on L2. During this period of time, the status of the L2 deposit transaction is xref:architecture:transactions/lifecycle.adoc#not_received[`NOT_RECEIVED`].
...
The status of the deposit request is now xref:architecture:transactions/lifecycle.adoc#accepted_on_l2[`ACCEPTED_ON_L2`].
...
* xref:architecture:network-architecture/l1<>l2-messaging.adoc[L1-L2 messaging]
