...
Much like other blockchains, a Starknet block is a list of transactions and a block header. The block header contains xref:whats_in_a_block[a lot of information] about the block, including a xref:block_hash[hash] of its parent, a xref:state_diff_commitment[commitment] to their state diff, and xref:transactions_events_receipts_commitments[commitments] the transactions, transactions receipts, and events included in the block.
 
== Contents of a block header 

A Starknet block header contains the following fields:
...
A state diff commitment is defined by:
...
Commitments to transactions, transactions receipts, and events are all roots of height-64 binary Merkle Patricia tries where the leaf at index `i` corresponds to:
...
* For transaction receipts: The xref:#receipt_hash[hash] of the ``i``'th transaction receipt included in the block
* For events: The xref:#event_hash[hash] of the ``i``'th emitted event included in the block
