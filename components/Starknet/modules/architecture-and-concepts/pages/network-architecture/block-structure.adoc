...
== Introduction
Starknet blocks are the core units of execution and state progression on the Starknet network, with each block comprised of a list of transactions and a block header. The block header contains xref:whats_in_a_block[a lot of information] about the block, including a hash of its parent and a commitment to their state diff, as well as commitments the transactions, transactions receipts, and events included in the block.
