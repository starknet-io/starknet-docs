...
Starknet is a Validity Rollup, which means that after consolidating and proving a set of Layer 2 changes, it updates the latest proven L2 state on L1. Alongside the proof, it publishes the state diff on L1. The state diff is the difference between the previous and new states, allowing anyone monitoring Ethereum can use this data to reconstruct the current state of Starknet.
