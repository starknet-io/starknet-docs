...
* Whenever a non-indexed storage key, it is mapped to the current value of the counter, and the counter is increased.
+
[NOTE]
====
Storage keys that require at most 127 bits are not mapped and continue to be referred to directly.

Indexing contract address and class hashes is planned for future versions.
====
