...
To mitigate this, SNOS has been modified in version 0.14.0 to receive multiple blocks as inputs, which also amortizes other non-negligible costs over a larger number of blocks. For example, if both block 10 and 11 use the same contract, SNOS loads and hashes its code only once instead of twice. The same is true for reading from storage and updating the Merkle Patricia tries.
