...
| Running a full node
| Follow the integration guide of any of the xref:ecosystem:fullnodes-rpc-providers.adoc[Starknet full node implementations]
|

| Attesting to blocks
| Follow the instruction of the https://github.com/eqlabs/starknet-validator-attestation/tree/main[Starknet validator attestation tool^]
|
...
a| * You must first approve the transfer of the amount of STRK tokens to be staked to the staking contract by invoking the STRK contract's `approve` function
