...
Starting from Starknet version 0.14.0, block times on Starknet are substantially shorter, leading to substantially smaller blocks. Every run of SNOS on a block also incurs a fixed cost of one recursion, which is independent of the size of the block. Therefore, if one produces e.g. twice as many blocks, one also doubles the accompanied fixed cost.

To mitigate this, SNOS has been modified in version 0.14.0 to receive multiple blocks as inputs, which also amortizes other non-negligible costs over a larger number of blocks. For example, if both block 10 and 11 use the same contract, SNOS loads and hashes its code only once instead of twice.
