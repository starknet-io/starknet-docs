...
[WARNING]
====
Deprecated `INVOKE` v0 transactions can call the `+__execute__+` function of all contracts. To mitigate this risk, `+__execute__+` calls should explicitly assert that the transaction version is 1 or above.
====
