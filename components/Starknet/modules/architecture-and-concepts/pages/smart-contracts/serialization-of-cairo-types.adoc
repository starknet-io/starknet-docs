...
The field element (`felt252`), which contains 252 bits, is the only actual type in the Cairo VM. So xref:data_types_of_252_bits_or_less[all data types that fit in 252 bits] are represented by a single felt, and xref:data_types_greater_than_252_bits[all data types that are larger than 252 bits], such as `u256` or arrays, are ultimately represented by a list of felts. In order to interact with a contract, you need to know how to encode its arguments as a list of felts so you can correctly formulate the calldata in the transaction.

Using the https://foundry-rs.github.io/starknet-foundry/starknet/calldata-transformation.html#using---arguments[`--argument` flag^] of xref:tools:core-tools.adoc#starknet-foundry[Starknet Foundry's `sncast`] is highly recommend to simply serialization, especially for working with complicated data structures. From `sncast` v0.43.0, the results of `call` are also parsed into readable information, demonstrated in the following example:
[source,terminal]
----
sncast call \
...
[TIP]
====
``sncast``'s `--arguments` flag is highly recommend to anyone who is struggling with calldata serialization.
====

xref:tools:interacting-with-starknet.adoc#sdks[Starknet SDKs], such as starknet.js, also encode the calldata for you, allowing you to simply specify any type while the SDK takes care of the serialization and encoding.

.Additional resources

* https://book.cairo-lang.org/ch02-02-data-types.html#integer-types[Integer types^] in _The Cairo Programming Language_

== Serialization of data types using 252 bits or less

For each of the following types, the value is serialized as a single-member list that contains one `felt252` value:
...
* Unsigned integers larger than 252 bits: xref:serialization_of_u256[`u256`] and xref:serialization_of_u512[`u512`]
* xref:serialization_of_arrays_and_spans[arrays and spans]
* xref:serialization_of_enums[enums]
* xref:serialization_of_structs_and_tuples[structs and tuples]
* xref:serialization_of_byte_arrays[`ByteArray`] (which represents strings)
