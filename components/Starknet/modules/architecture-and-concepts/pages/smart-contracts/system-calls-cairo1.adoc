...
[id="secp256k1"]
== `Secp256k1 System Calls`
The secp256k1 curve is commonly used in cryptographic applications such as Bitcoin and Ethereum.

Note that instead of using these syscalls directly, it is recommended to use the traits and implementations provided in the link: https://github.com/starkware-libs/cairo/blob/67c6eff9c276d11bd1cc903d7a3981d8d0eb2fa2/corelib/src/starknet/secp256_trait.cairo[secp256_trait] module which provide a more convenient interface and handle the syscalls under the hood.
...
Scalar Field: n = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
----

[id="secp256k1_new_syscall"]
...
[horizontal,labelwidth=35]
`x: u256`:: The x-coordinate of the point.
`y: u256`:: The y-coordinate of the point.
...
link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256k1.cairo#74[`secp256k1_new_syscall`]

[discrete]
=== Example for `secp256k1_new_syscall`
...
[horizontal,labelwidth=35]
`p0: Secp256k1Point`:: The first point on the curve.
`p1: Secp256k1Point`:: The second point on the curve.
...
link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256k1.cairo#L79[`secp256k1_add_syscall`]

[discrete]
=== Example for `secp256k1_add_syscall`
...
[horizontal,labelwidth=35]
`p: Secp256k1Point`:: The point to be multiplied.
`scalar: u256`:: The scalar value to multiply the point by.
...
link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256k1.cairo#L84[`secp256k1_mul_syscall`]

[discrete]
=== Example for `secp256k1_mul_syscall`
...
[id="secp256k1_get_point_from_x_syscall"]
...
Recovers a point on the curve given its x-coordinate and y-parity. Since the curve equation y² = x³ + 7 has two solutions for y given x, the y_parity parameter determines which y value to use.
...
[horizontal,labelwidth=35]
`x: u256`:: The x-coordinate of the point.
`y_parity: bool`:: If true, choose the odd y value; if false, choose the even y value.
...
link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256k1.cairo#L102[`secp256k1_get_point_from_x_syscall`]

[discrete]
=== Example for `secp256k1_get_point_from_x_syscall`
...
[horizontal,labelwidth=35]
`p: Secp256k1Point`:: The point whose coordinates are to be retrieved.
...
link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256k1.cairo#L107[`secp256k1_get_xy_syscall`]

[discrete]
=== Example for `secp256k1_get_xy_syscall`
...
This system call is particularly useful when implementing traditional PKI or secure communication functionality, as secp256r1 is widely used in TLS, secure messaging, and government standards.
...
# Parameters
Base Field:    p = 0xFFFFFFFF00000001000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF
Scalar Field: n = 0xFFFFFFFF00000000FFFFFFFFFFFFFFFFBCE6FAADA7179E84F3B9CAC2FC632551
a = -3
b = 0x5AC635D8AA3A93E7B3EBBD55769886BC651D06B0CC53B0F63BCE3C3E27D2604B
----

Note that instead of using this syscall directly, it is recommended to use the functions provided in link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256r1.cairo[`secp256r1.cairo`] which provide a more convenient interface and handle the syscall under the hood.

These include:
- link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256r1.cairo#L78[secp256r1_add_syscall]
- link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256r1.cairo#L83[secp256r1_mul_syscall]
- link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256r1.cairo#L101[secp256r1_get_point_from_x_syscall]
- link:https://github.com/starkware-libs/cairo/blob/main/corelib/src/starknet/secp256r1.cairo#L106[secp256r1_get_xy_syscall]
...
link:https://github.com/starkware-libs/cairo/blob/67c6eff9c276d11bd1cc903d7a3981d8d0eb2fa2/corelib/src/starknet/secp256r1.cairo[`secp256r1.cairo`^]

[discrete]
=== Example

The following example demonstrates how to create and validate a point on the secp256r1 curve using the recommended functions from link:https://github.com/starkware-libs/cairo/blob/67c6eff9c276d11bd1cc903d7a3981d8d0eb2fa2/corelib/src/starknet/secp256r1.cairo[`secp256r1.cairo`^]:
