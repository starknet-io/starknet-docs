...
Note that instead of using these syscalls directly, it is recommended to use the traits and implementations provided in the https://github.com/starkware-libs/cairo/blob/67c6eff9c276d11bd1cc903d7a3981d8d0eb2fa2/corelib/src/starknet/secp256_trait.cairo[`secp256_trait`^] module which provide a more convenient interface and handle the syscalls under the hood.
